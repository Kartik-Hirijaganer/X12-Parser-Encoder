...
The Eligibility Workbench is a task-driven healthcare tool — not a marketing site, not a dashboard for power users. The design is light-mode-first, rooted in retail-grade clarity: a generous warm cream canvas where content, status, and actions are instantly legible. Every pixel serves the workflow. Non-technical users should feel confident, not overwhelmed.

The surface strategy is predominantly light — warm primary surfaces for browsing and configuration, secondary warm surfaces for grouped sections, and a controlled dark palette reserved exclusively for immersive data moments (raw X12 preview, code blocks). This is a tool people use once a week for 20 minutes; it must be self-evident every time they return.

The color system is built around a single Mastercard-inspired action orange (`--color-action-500`) that drives all CTAs, links, and interactive focus. Status is communicated through a disciplined four-color semantic system: green for active/success, Mastercard red for inactive/error, amber for warnings/corrections, and slate for not-found/unknown. These status colors appear only in badges, stat cards, and table cells — never in chrome or navigation.
...
Buttons are pill-shaped, fully rounded, and unmistakable. Cards have generous 20–24px radius. The overall impression is clean, approachable, and medical-grade trustworthy — the visual equivalent of a well-organized clinic reception desk.

**Key characteristics:**

- Light-mode-first: warm cream canvas with soft secondary sections.
- Single warm action orange for all interactive elements — no competing accent colors.
...
- Cards at 20–24px radius — smooth, approachable, never sharp.
...
### Primary action — one warm accent, one job
...
Neutral surfaces go from warm cream (page canvas and cards) through soft warm secondary sections and web wash to near black (reserved for code / X12 preview backgrounds only). A full-screen modal overlay tints the page at 50% opacity.

### Text & content

Five text roles: primary (main headings and body — warmer than black), secondary (descriptions, labels, help text), tertiary (placeholders, timestamps, metadata), disabled (inactive labels), and inverse (text on action or dark backgrounds).
...
Default borders on inputs, cards, and table rows; subtle borders for minimal separation; a strong variant for focused input strokes behind the focus ring; a divider tone for horizontal rules; and a semi-transparent focus ring (3px, action orange at ~28% alpha).

### Gradients

A near-black `--color-surface-dark` -> `--color-surface-dark-end` gradient for X12 raw preview backgrounds, and a warm-surface -> wash gradient for drag-and-drop target areas.
...
- **Primary (pill).** Action-orange background, inverse text, no border, fully rounded (`--radius-pill`). Padding `10px 24px`, label weight 500. Hover darkens to `--color-action-600` with a subtle lift. Pressed uses `--color-action-700`. Focus ring uses `--color-focus-ring`. **Use for:** Process, Download X12, Save Changes, Export.
- **Secondary (outlined pill).** Transparent background, primary-text label, 2px default border. Hover tints to the secondary warm surface and darkens the border. **Use for:** Cancel, Upload Another, Copy to Clipboard.
- **Ghost.** Transparent background, action-orange label, 8px radius. Hover fills with `--color-action-50`. **Use for:** View Details, inline links, navigation.
...
- **Content card (generic).** Warm primary surface, 1px default border, `--radius-xl` (20px), `--shadow-sm`. Padding 20px.
- **Action card (home page).** Warm primary surface, 1px default border, `--radius-2xl` (24px), `--shadow-md`. Hover translates up 2px and raises to `--shadow-lg`. 48px action-color icon top-aligned; 20px/600 title; 14px/400 secondary description (max 3 lines); ghost button or file input at bottom.
- **Stat card (dashboard).** Uses the status 50-background and 200-border tokens. 20px radius, 20px padding. Stat value 32px/600 in the status 500 color; label 14px/500 in primary text. No shadow — color differentiation is sufficient.
...
- Required field asterisks use `--color-required-asterisk`.
...
- Focus: border switches to action orange with the tokenized focus ring.
...
- **SNIP level badge.** Secondary warm background, slate text, 1px default border, 6px radius (`--radius-sm`), mono 12px/500. Shown as `SNIP 1`, `SNIP 2`, etc.

### Tables

- Header: tertiary warm background, 13px/600 uppercase with 0.04em tracking, 2px default border-bottom.
- Rows: alternate primary surface / `--color-row-alt`. 1px subtle border between rows.
- Cell padding: `12px 16px`, 14px body text.
- Hover tint: secondary warm surface. Selected or expanded row: action-50 with a 3px action-color left border.
- Expandable row: right-chevron icon; rotates 90° when expanded.
- Excel eligibility exports tint error rows with `FFF1F2`, matching `--color-inactive-50`;
  keep this server-side workbook fill aligned when the inactive background token changes.

### Navigation

- White background, sticky top, 56px tall, 1px default border-bottom.
- Logo/title: 18px/600, left-aligned. Links: 14px/500 secondary. Active link: action orange with a 2px bottom border. The icon-only Home link is navigation only, appears away from `/`, and uses tooltip text `Home`.
...
- Hover / drag-over: background shifts to `--color-action-50`, border turns solid action orange.
...
- Settings: single-column on mobile, two-column card grid on medium and larger screens, max-width `--layout-settings-max`, centered.
...
| `--radius-xl` (20px) | Content cards, stat cards, drop zone |
| `--radius-2xl` (24px) | Action cards (home page) |
...
The Eligibility Workbench uses a flat elevation model. Most differentiation comes from background shifts (primary warm surface -> secondary warm surface) and borders, not shadows. Shadows appear only on interactive cards (home page) and floating elements (modals, dropdowns). This keeps the interface grounded and utilitarian — appropriate for a healthcare tool where trust matters more than flair.
...
- Don't introduce additional accent colors beyond the warm action token.
...
- **Settings form.** Single column below 768px; two-column card grid at 768px and above.
...
- Backgrounds → `--color-surface-primary` (warm cream), `--color-surface-secondary` (secondary warm surface), `--color-surface-tertiary` (tertiary warm surface), `--color-surface-wash` (wash), `--color-surface-dark` (code)
...
- Motion → `--duration-fast` / `-normal` / `-slow` / `-route`, `--motion-fade-in`, `--motion-route-slide`, `--ease-out`

### Example component prompts

- "Create the Home Page with 3 action cards in a grid. Use the `Card` primitive with `variant='action'`. Each card: 48px action-color icon at top, 20px/600 title, 14px/400 secondary description, ghost button at bottom. Below the cards, render `FileUpload` in `dropzone` variant."
...
- "Build the Settings page as a mobile single-column, medium-plus two-column card grid. Group headers at 20px/600 with a subtle bottom border. Use `UnsavedChangesBar` for Save and Discard."
...
1. Always use `--color-action-500` for interactive elements — no other accent variants in chrome.
...
5. Pill buttons for CTAs, 8px radius for inputs, 20–24px for cards.
...
8. Every interactive element needs a visible focus state using `--color-focus-ring`.
