...
describe('nextIsaControlNumber', () => {
  it('returns null when last is null', () => {
    expect(nextIsaControlNumber(null)).toBeNull()
  })

  it('increments by 1 when last is within range', () => {
    expect(nextIsaControlNumber(1)).toBe(2)
    expect(nextIsaControlNumber(19)).toBe(20)
    expect(nextIsaControlNumber(100)).toBe(101)
    expect(nextIsaControlNumber(999)).toBe(1000)
  })

  it('returns null when last is at max', () => {
    expect(nextIsaControlNumber(MAX_ISA_CONTROL_NUMBER)).toBeNull()
  })

  it('returns null when last exceeds max', () => {
    expect(nextIsaControlNumber(MAX_ISA_CONTROL_NUMBER + 1)).toBeNull()
  })
...
describe('hasUsableNextIsaControlNumber', () => {
  it('is false when unset or exhausted', () => {
    expect(hasUsableNextIsaControlNumber(null)).toBe(false)
    expect(hasUsableNextIsaControlNumber(999_999_999)).toBe(false)
  })

  it('is true when the next ICN is available', () => {
    expect(hasUsableNextIsaControlNumber(19)).toBe(true)
  })
})

describe('formatIsaControlNumber', () => {
  it('formats ICNs as nine digits', () => {
    expect(formatIsaControlNumber(20)).toBe('000000020')
  })
})
